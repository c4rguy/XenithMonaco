'use strict';var core=require('@monacopilot/core');var k=n=>!n||n.length===0?"":n.length===1?n[0]:`${n.slice(0,-1).join(", ")} and ${n.slice(-1)}`;var S=(n,e,o={})=>{if(e<=0)return "";let t=n.split(`
`),r=t.length;if(e>=r)return n;if(o.truncateDirection==="keepEnd"){let s=t.slice(-e);return s.every(l=>l==="")?`
`.repeat(e):s.join(`
`)}let i=t.slice(0,e);return i.every(s=>s==="")?`
`.repeat(e):i.join(`
`)};var re="<|developer_cursor_is_here|>",B=n=>({instruction:ie(),context:se(n),fileContent:ae(n)}),ie=()=>"Provide concise and readable code completions that are syntactically and logically accurate, and seamlessly integrate with the existing context. Output only the raw code to be inserted at the cursor location without any additional text, comments, or text before or after the cursor.",se=n=>{let{technologies:e=[],filename:o,relatedFiles:t=[],language:r}=n,i=k([r,...e].filter(a=>!!a)),s=t.length===0?"":t.map(({path:a,content:p})=>`### ${a}
${p}`).join(`
